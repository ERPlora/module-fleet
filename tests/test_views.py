...
class TestVehicleViews:
    ...
    def test_list_loads(self, auth_client):
        ...
        url = reverse('fleet:vehicles_list')
        response = auth_client.get(url)
        assert response.status_code == 200
    ...
    def test_edit_form_loads(self, auth_client, vehicle):
        """Test edit form loads."""
        url = reverse('fleet:vehicle_edit', args=[vehicle.pk])
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_edit_post(self, auth_client, vehicle):
    ...
    def test_delete(self, auth_client, vehicle):
        """Test soft delete via POST."""
        url = reverse('fleet:vehicle_delete', args=[vehicle.pk])
        response = auth_client.post(url)
        assert response.status_code == 200
        vehicle.refresh_from_db()
        assert vehicle.is_deleted is True

    def test_bulk_delete(self, auth_client, vehicle):
        """Test bulk delete."""
        url = reverse('fleet:vehicles_bulk_action')
        response = auth_client.post(url, {'ids': str(vehicle.pk), 'action': 'delete'})
        assert response.status_code == 200
        vehicle.refresh_from_db()
        assert vehicle.is_deleted is True
